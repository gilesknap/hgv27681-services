...
if [[ ${HOST_NETWORK} == "NO" ]]; then
  ...
  export EPICS_CA_NAME_SERVERS="${IPS:-127.0.0.1}"
  echo "EPICS_CA_NAME_SERVERS set to: ${EPICS_CA_NAME_SERVERS}"
fi
...
/epics/ca-gateway/bin/linux-x86_64/gateway -sport ${CA_SERVER_PORT} \
